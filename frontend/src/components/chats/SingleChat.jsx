...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  const { user, selectChat, setSelectChat, notification, setNotification } =
    useChatState();
  ...
  const fetchAllMessages = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(
        `${REQUEST_URL}/message/${selectChat._id}`,
        GET_CONFIG
      );
  ...
  useEffect(() => {
    socket.on("message received", (newMessageRecieved) => {
      if (
        !selectChatComaprare ||
        selectChatComaprare?._id !== newMessageRecieved?.chat?._id
      ) {
